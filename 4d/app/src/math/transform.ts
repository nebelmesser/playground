...
export function transform4(vertex: Vec4, ctx: TransformContext): Vec3 {
  ...
  const wFactor = ctx.projectionDistance / Math.max(0.2, ctx.projectionDistance - w);
  return [x * wFactor, y * wFactor, z * wFactor];
}

export function rotate3Into(
  x0: number,
  y0: number,
  z0: number,
  w0: number,
  ...
  const [x, y, z] = rotate4([x0, y0, z0, w0], ctx.angles, ctx.orbit);
  out[0] = x;
  out[1] = y;
  out[2] = z;
  return out;
